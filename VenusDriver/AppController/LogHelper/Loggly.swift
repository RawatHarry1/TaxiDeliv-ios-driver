...
struct ColorLog {
    ...
    static func red<T>(object: T) {
        printDebug("\(ESCAPE)31m\(object)\(RESET)")
    }

    static func green<T>(object: T) {
        printDebug("\(ESCAPE)32m\(object)\(RESET)")
    }

    static func blue<T>(object: T) {
        printDebug("\(ESCAPE)34m\(object)\(RESET)")
    }

    static func yellow<T>(object: T) {
        printDebug("\(ESCAPE)33m\(object)\(RESET)")
    }

    static func purple<T>(object: T) {
        printDebug("\(ESCAPE)35m\(object)\(RESET)")
    }
    ...
    open var directory : String = Loggly.defaultDirectory() {
        didSet {
            guard directory.count > 0 else {
                printDebug("Error changing logger directory. The value \(directory) is not valid. Using default value instead")
    ...
    open func readFromPath(filePath: String) -> NSMutableDictionary{
        ...
        if fileManager.fileExists(atPath: filePath) {
            do {
                ...
                /* error handling here */
                printDebug("Error while read csv: \(error)")
            }
        }
        return output;
    }
    ...
    open func write(_ type: LogType, text: String) {
        ...
        if !fileManager.fileExists(atPath: path) {
            do {
                ...
                printDebug("error getting wriet string: \(error)")
            }
        }
        if let fileHandle = FileHandle(forWritingAtPath: path) {
...
/// free function to make writing to the log with Log type
